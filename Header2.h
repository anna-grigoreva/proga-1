...
public:
    ...
    friend Matrix operator-(const Matrix& a, const Matrix& b);
    friend Matrix operator*(const Matrix& a, int ch);
    friend Matrix operator*(int ch, const Matrix& a);
    friend Matrix operator*(const Matrix& a, const Matrix& b);
...
Matrix operator*(const Matrix& a, int ch)
{
    setlocale(LC_ALL, "rus");
...
Matrix operator*(int ch, const Matrix& a)
{
    setlocale(LC_ALL, "rus");
    cout << "Произведение матрицы и числа равно:" << endl;
    Matrix result(a.row, a.column);
...
void menu1()
{
    ...
    cout << "9. Вычислить определитель матрицы (методом Гауса)\n";
